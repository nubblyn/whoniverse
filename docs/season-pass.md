...
anything is. How the project is put together (the parts, the ledger's columns, file
names, the addon entry, what builds what, every script) is in
[architecture.md](architecture.md); read that first. Here, Part B is the toolbox, one
entry per tool. Part C is where episodes and their facts live. Part D is the rules the
files must meet. Part E is the eight steps with the exact commands. Part G is what
changes when a season is replaced rather than added. **Part H is the checklist every
finished season reports against, and Part I the rules settled while the passes were
running; read those two before starting a season.**
...
- **The archive files are bilingual.** Each is the whole English track followed by the
  whole track again in Thai, numbered from 1 again. The word test passes on the English
  half, so the first run of 23 September shipped Thai cues inside 20 files before a scan
  caught them. `build_subs.py` now keeps only cues whose letters are all Latin, and
  drops empty cues and Whisper's stacked echoes (a line repeated while the previous copy
  is still on screen) from every file it writes. Scan the output for non-Latin letters,
  empty cues and repeats before any upload, whatever the source.
...
- **Encode** (YouTube VP9/AV1, or anything that must change resolution):
  ...
  subtitle, and smuggles PGS back in as `bin_data`. S16E05 needed a second pass for
  exactly this.
...
- `C:/Program Files/7-Zip/7z.exe`. TorBox delivers each torrent as one zip in
  `~/Downloads`. A Collection set is 110 to 230 GB, so it is never unpacked whole:
  `collsel.py --zip` reads the listing and 7-Zip extracts only the chosen files (Part C).
- Pass Windows paths: `7z t "$(cygpath -w "$z")"`. Git Bash converts a POSIX path only
  when the name has no brackets, so `[TGx]` names failed in under a second as "FAILED
  integrity test". A real test on 6 GB takes ten to twenty seconds; an instant failure is
  a path bug.
...
Every script in the repo, with what it does, is in architecture.md, *The scripts*.
...
| **Internet Archive** | Classic DVD ISOs (`doctor-who-season-1-dvd`, `DoctorWhoLostInTime` NTSC, `doctor-who-galaxy-4-region-free`…), the Wilderness webcast reconstructions (`doctor-who-bbci-webcasts-death-comes-to-time-real-time-shada`), 614 Classic .srt (`doctor-who-1963_20251231`) | HTTP download, or `rclone`'s IA backend | no Blu-ray sets there; "1280x720" webcasts are upscales; every complete-run item is a mixed compilation |
...
`scripts/media/dvd/dvdrip.py rip <season> <iso folder>` takes every episode-length title
out of the ISOs with MakeMKV, `dvdrip.py match <season>` names each by audio against the
bucket copy and writes `chosen_s<season>.tsv`, and `collstage.py` stages and verifies
from there exactly as for a Blu-ray set.
...
read the dry run's picks against the zip listing before `--apply`, every time.
...
zips plus their episodes will not fit the disk.
...
- **A large offset means a different cut, so check the runtime.** That held copy
  ...
  Seasons 24 and 25 went further: every season 24 file in the bucket, and all three of
  season 25's Silver Nemesis, was a longer edit, one to four minutes over broadcast, with
  scenes inserted all the way through. That breaks the single 180 s window, so three
  season 24 episodes read as mismatches. The test that settles it is four 120 s windows
  of the new file, each searched across the whole held copy: the right episode matches
  every window, at offsets that grow down the file. Then the runtimes against Wikipedia
  confirm the new file is the broadcast one.
...
| **IMDb** / **Cinemeta** (`v3-cinemeta.strem.io/meta/series/<tt>.json`) | the `imdb` mapping; `scripts/map-imdb.js` |
...
- Minisodes are numbered into the series by story placement; a Lockdown short after The
  ...
  24 April 2020 (architecture.md, *An episode in the addon*).
...
`ffprobe` each of the season's stream URLs against the B2 origin
(`f003.backblazeb2.com`, since Cloudflare refuses tools) and compare codec, resolution,
frame rate, audio and subtitle tracks against `have` and the entry's `audio` flag. A range
probe costs a few seconds a file and downloads nothing:

```
ffprobe -v error -show_entries stream=codec_type,codec_name,width,height,r_frame_rate,channels:stream_tags=language \
  -of compact "https://f003.backblazeb2.com/file/whoniverse/<folder>/season_N/<stem>.mkv"
```

Count audio tracks with `mkvmerge -J` rather than ffprobe, which lists a DTS-HD MA track
and its core as two (Part C).
...
is not being run. Editing a TSV with a script that rebuilds whole lines is how the tab
goes missing, so run this after any such edit.
...
1. Download: magnets into TorBox or qBittorrent. A TorBox zip of a whole set goes
   through `collsel.py --zip` and a chosen-file extract (Part C), never a full unpack.
   YouTube via `python -m yt_dlp`.
...
4. Subtitles. The release's own tracks stay inside the file, in whatever format they
   ...
   - **The file carries a subtitle track of any kind, text or bitmap: there is no
     ...
     after the deploy. Every series but New Who is generated by `build-series.js`, which
     links any `.srt` it finds beside a video, so for those the old sidecar is deleted
     from the bucket **before** regenerating, not after the deploy; the old file loses its
     sidecar for the minutes until the deploy lands. A sidecar from a different release is still a removal, however
...
Before the upload, `ready.py` checks the folder. Of its four checks, `hvc1` tagging and
faststart are MP4 properties a shipped MKV has neither of; the other two, the audio codec
(which decides whether the entry needs an `audio` flag) and whether each `.srt` is really
text, apply to everything. It takes `.mkv` and reads another tree through `READY_CONTENT`:
...
at a time, and nothing else touching the files it is working on.
...
**`PYTHONIOENCODING=utf-8` on every python call in Git Bash**, or a script dies
printing a title with a non-ASCII character in it. `pq.py` throws the same error on a broken pipe
