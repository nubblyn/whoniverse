...
ledger/series/*.tsv + bucket ──► scripts/build-series.js --write ──► every data/*.js except New Who's
...
Only `data/new-who.js` is edited by hand. Every other series file, Classic Who and the
Wilderness Years included, is generated by `build-series.js` from its ledger file and the
bucket listing, so a change goes into the ledger and is regenerated, never typed into the
data file; the chronology is generated by `build-chronology.js`. The generated files carry
no IMDb ids: the addon answers only its own `whoniverse_` ids, so an IMDb id there does
nothing.
...
| `scripts/build-series.js --write` | regenerates every data file but New Who's from the ledger and the bucket |
