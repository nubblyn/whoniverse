...
function landingPage(manifest, baseUrl, bucket) {
  ...
  const segments = [
    ...
    // Only while something is still to come; an empty "Coming" read as a fault.
    ...(queued.length ? [`<b>Coming</b> ${queued.map((p) => esc(p.entry.name)).join(', ')}`] : []),
  ...
  <div class="col">
    ...
    <div class="intro">
      ...
      <div class="stack rise" style="--d:.2s">
        <p class="blurb">Doctor Who and its spin-offs, 1963 to now, in original UK broadcast order. Every special, minisode and prequel sits inside its season where its story belongs.</p>
        <div class="row">
    ...
    <div class="ledger rise" style="--d:.65s">
      <div class="ledger-text">
        <h2>Check every episode.</h2>
        <p>Every item in the catalogue, with what we hold, the best copy found and what is still missing. It is the same ledger the addon is built from.</p>
        <div class="tally">
