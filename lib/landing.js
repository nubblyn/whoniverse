...
/* Selected fills rather than outlines: a brighter border read as a hover state
   ...
   the chip keeps its size and shows no outline. Nothing here transitions: a
   filter is on or off, and crossfading the fill against the label made the
   text wash out mid-change instead of switching cleanly. */
...
/* The summary slides open: a grid row that grows from 0fr to 1fr. The box and
   ...
   it was still being squeezed. The text now waits for the box to be most of the
   way open and then fades, and only fades: sliding it as well meant its glyphs
   were re-rasterised at fractional offsets every frame, which read as the text
   shuffling itself into position. Closing reverses the order, text first. */
...
.ep.open .ep-o p{opacity:1;transition:opacity .16s ease .04s}
