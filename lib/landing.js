...
<link rel="icon" href="${esc(manifest.logo)}">
...
/* Where it runs: the two players' wordmarks, framed as the earlier design had
   them, under a label in the page's muted caps. */
.works{display:flex;align-items:center;gap:16px;color:var(--mute);font-weight:700}
.works img{display:block;width:220px;height:50px}
...
.chip{background:none;border:1px solid var(--line);color:var(--mute);border-radius:4px;padding:9px 12px;
  ...
  gap:8px;align-items:baseline;transition:border-color .15s,color .15s}
...
/* The summary slides open: a grid row that grows from 0fr to 1fr. */
.ep-o{grid-column:1/-1;display:grid;grid-template-rows:0fr;opacity:0;margin-top:0;
  transition:grid-template-rows .3s var(--ease),opacity .3s var(--ease),margin-top .3s var(--ease)}
.ep-o p{margin:0;min-height:0;overflow:hidden;font-size:15px;line-height:1.5;color:rgba(255,255,255,.8)}
.ep.open .ep-o{grid-template-rows:1fr;opacity:1;margin-top:10px}
@media(prefers-reduced-motion:reduce){.ep-o{transition:none}}
  ...
  <div class="col">
    ...
    <div class="intro">
      ...
      <div class="stack rise" style="--d:.2s">
        ...
        <div class="works">
          <span class="caps">Works in</span>
          <img src="${artUrl('works-in.svg')}" alt="Stremio and Nuvio" width="220" height="50">
        </div>
      </div>
    </div>
    ...
    <div class="foot rise" style="--d:.75s">
      ...
      <div class="foot-text caps">
        <p>Whoniverse ${esc(manifest.version)} &middot; <a href="${esc(manifestUrl)}">manifest.json</a> &middot; ${n(live)} of ${n(total)} episodes available.</p>
...
(function () {
  ...
  function row(s, e) {
    var th = e.i ? '<img loading="lazy" decoding="async" src="' + esc(e.i) + '" alt="" width="320" height="180">' : '<span class="caps">' + num(s, e) + '</span>';
    return '<div class="ep ' + (e.a ? 'on' : 'off') + (e.o ? ' has-o' : '') + '"' + (e.o ? ' role="button" tabindex="0" aria-expanded="false"' : '') + '>' +
      '<div class="ep-th">' + th + '</div>' +
      '<div class="ep-main">' +
