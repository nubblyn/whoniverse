...
const manifest = {
  ...
  description:
    ...
    'and prequel inside its season. New Who is complete. Classic Who, the 1996 ' +
    'film, Torchwood, The Sarah Jane Adventures, Class and The War Between the ' +
    'Land and the Sea are on the way.',
  logo: ADDON_LOGO,
  types: ['series', 'movie'],
  ...
  // One row per type that has something to play: a client cannot mix films
  ...
  ].filter((c) => playableSeries().some((s) => typeOf(s) === c.type)),
  behaviorHints: { configurable: false, adult: false },
  stremioAddonsConfig: {
    issuer: 'https://stremio-addons.net',
    signature:
      'eyJhbGciOiJkaXIiLCJlbmMiOiJBMTI4Q0JDLUhTMjU2In0..gTvVWDhy-2sJkHP0suaDVg.DyQoj_Hr8tq-zvSJIfx8JeK_s1DCI81ypxyouv5fyGVktTkJMQsnxTlX8m3XF4Vbtf0nqvGAT7JsdvZNvxLscbof-YSRxelrWMl89X95jAE7M7z7wFXD2nid_rg0fY-C.GauSecXP5voj_lhR6K0Nkg',
  },
};

const builder = new addonBuilder(manifest);

// Only what plays is listed. The rest of the Whoniverse is catalogued and
// shown on the website, and joins here the day its files land.
builder.defineCatalogHandler(async (args) => {
  if (args.id !== CATALOG_ID && args.id !== MOVIE_CATALOG_ID) return { metas: [] };
  return { metas: playableSeries().filter((s) => typeOf(s) === args.type).map(toCatalogMeta) };
});

builder.defineMetaHandler(async (args) => {
  const entry = playableSeries().find((s) => s.stremioId === args.id);
