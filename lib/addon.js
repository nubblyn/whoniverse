...
function statusSentence() {
  ...
  const soon = populatedSeries()
    ...
    .map((s) => s.name);
  const parts = [];
  // Once everything plays, the description above has already named every
  // series, so saying their names again is only length.
  if (playing.length && !soon.length) parts.push('All of it plays now.');
  else if (playing.length) parts.push(`${andList(playing)} play now.`);
  ...
  if (populatedSeries().some((s) => s.key === chronology)) {
    parts.push('The Complete Chronology runs every series as one list.');
  }
  return parts.join(' ');
}
