...
// A corrected subtitle keeps its name, so without this the reply sits in the
// edge cache for a day and viewers keep the old text. The data carries a hash
// of the file in the query, which makes a changed file a changed URL; the
// bucket never sees it, because only the path is forwarded.
function splitVersion(rel) {
  const q = rel.indexOf('?');
  return q === -1 ? [rel, ''] : [rel.slice(0, q), rel.slice(q)];
}
...
async function serveSubtitle(path, res) {
  // The version is for the caches between here and the viewer. Only the path
  // goes upstream.
  const [rel] = splitVersion(decodeURIComponent(path.slice(ROUTE.length)));
...
module.exports = { subtitlesFor, isSubtitlePath, serveSubtitle };
