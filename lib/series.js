...
// The hash is written by scripts/build-art.js. Without it a replaced logo sits
// behind Cloudflare's cache for four hours; with it the URL changes when the
// file does. Absent on a fresh clone that has not built art yet.
let logoVersion = '';
try { logoVersion = `?v=${require('../data/art-version.json').addonLogo}`; } catch { /* not built yet */ }
const ADDON_LOGO = `${CDN_ART}/addon-logo.png${logoVersion}`;

/** Poster, background and logo for a series, all from the bucket. */
function bucketArt(key) {
  return {
    poster: `${CDN_ART}/poster/${key}.jpg`,
    background: `${CDN_ART}/background/${key}.jpg`,
    logo: `${CDN_ART}/logo/${key}.png`,
  };
}
...
const series = [
  ...
  },
  {
    ...
    // A single film, filed under Stremio's movie type rather than as a one-episode series.
    type: 'movie',
    description:
